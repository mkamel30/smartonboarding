...
// Create a new batch
router.post('/', authorizeRole('BRANCH_SALES'), upload.single('waybillDoc'), async (req: any, res) => {
    try {
        ...
        const batch = await prisma.shipmentBatch.create({
            data: {
                ...
                requests: {
                    connect: idsArray.map((id: string) => ({ id }))
                }
            },
...
// Receive batch
router.patch('/:id/receive', authorizeRole('BRANCH_MGMT', 'ADMIN'), async (req: any, res) => {
    try {
        const { id } = req.params;
        const user = req.user;
