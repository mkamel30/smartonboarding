...
// Upload Sales Form
apiRouter.post('/upload-sales-form', authorizeRole('SALES_MGMT', 'ADMIN'), upload.single('salesForm'), async (req, res) => {
    try {
...
apiRouter.patch('/requests/:id/action', async (req, res) => {
    try {
        const { id } = req.params;
...
apiRouter.get('/pending-counts', async (req, res) => {
    try {
        const user = req.user;
        let directCount = 0;
        let shippingCount = 0;
