...
export async function processAction(requestId, action, user, payload = {}) {
    ...
    switch (request.stage) {
        ...
        case WORKFLOW_STAGES.BRANCH_MGMT_REVIEW:
            ...
            if (action === 'approve') {
                if (!kycType)
                    throw new Error('KYC Type is required for approval');
        ...
        case WORKFLOW_STAGES.SALES_MGMT_REVIEW:
            if (user.role !== ROLES.SALES_MGMT && user.role !== 'ADMIN')
                throw new Error('Unauthorized');
            if (action === 'approve') {
                if (!formFileId)
                    throw new Error('Signed form file ID is required');
        ...
        case WORKFLOW_STAGES.OPERATIONS_REVIEW:
            if (user.role !== ROLES.OPERATIONS && user.role !== 'ADMIN')
                throw new Error('Unauthorized');
        ...
        case WORKFLOW_STAGES.BANK_REVIEW:
            ...
            if (action === 'bank_approved') {
                if (!mid)
                    throw new Error('MID is required for bank approval');
