...
router.post('/login', async (req, res) => {
    try {
        ...
        if (user.mfaEnabled) {
            ...
            const tempToken = jwt.sign({ id: user.id, mfaVerified: false }, JWT_SECRET, { expiresIn: '5m' });
            return res.json({ mfaRequired: true, tempToken });
        }
        const token = jwt.sign({ id: user.id, mfaVerified: false }, JWT_SECRET, { expiresIn: JWT_EXPIRES_IN });
...
router.post('/verify-mfa', async (req, res) => {
    try {
        ...
        const token = jwt.sign({ id: user.id, mfaVerified: true }, JWT_SECRET, { expiresIn: JWT_EXPIRES_IN });
        const { passwordHash, mfaSecret, ...userSafe } = user;
        res.json({ token, user: userSafe });
    }
    catch (error) {
...
// Confirm MFA Setup
router.post('/mfa/verify-setup', authenticate, async (req, res) => {
    try {
